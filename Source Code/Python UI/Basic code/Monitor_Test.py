...
# text = Text(root)
#l = Label(root, text="This label is over all buttons").grid(row=8)
...
Weeks=['1', '2', '3','4','5','6','7']
Years=['1','2']
tkvar1.set('Select') # set the default option
if tkvar.get() == Minutes : 
    popupMenu1 = OptionMenu(mainframe, tkvar1, *Minutes)
if tkvar.get() == Seconds : 
    popupMenu1 = OptionMenu(mainframe, tkvar1, *Seconds)
if tkvar.get() == Days : 
    popupMenu1 = OptionMenu(mainframe, tkvar1, *Days) 
if tkvar.get() == Weeks : 
    popupMenu1 = OptionMenu(mainframe, tkvar1, *Weeks) 
if tkvar.get() == Years : 
    popupMenu1 = OptionMenu(mainframe, tkvar1, *Years) 


Label(mainframe, text="Poll Interval").grid(row = 13, column = 1)
popupMenu1.grid(row = 13, column =2)
...
Button(root, text='Quit', command=root.quit).grid(row=15, column=0, sticky=W, pady=4)
Button(root, text='Show', command=show_entry_fields).grid(row=15, column=1, sticky=W, pady=4)
root.mainloop()